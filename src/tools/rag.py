...
def _cache_get(key: Tuple) -> Optional[Tuple[List, List, str, List]]:
    entry = _context_cache.get(key)
    if entry is None:
        return None
    chunks, chunk_types_used, doc_summary, sources, ts = entry
    if time.time() - ts > _CACHE_TTL:
        del _context_cache[key]
        return None
    _context_cache.move_to_end(key)
    return chunks, chunk_types_used, doc_summary, sources


def _cache_set(key: Tuple, chunks: List, chunk_types_used: List, doc_summary: str, sources: List) -> None:
    if key in _context_cache:
        _context_cache.move_to_end(key)
    _context_cache[key] = (chunks, chunk_types_used, doc_summary, sources, time.time())
...
class RAGTool(BaseTool):
    ...
    def __init__(self):
        ...
        self.gemini_model = get_gemini_model("gemini-2.5-flash-lite")
        self.chroma = chromadb.PersistentClient(path=os.getenv("CHROMA_PATH", "./chroma_store"))
        self.embed_fn = None
    ...
    def run(self, input: Dict[str, Any]) -> ToolResult:
        ...
        collection_name = input.get("collection_name") or "research_papers"
        ...
        try:
            ...
            if cached is not None:
                chunks, chunk_types_used, doc_summary, sources = cached
                ...
                _cache_set(cache_key, chunks, chunk_types_used, doc_summary, sources)
